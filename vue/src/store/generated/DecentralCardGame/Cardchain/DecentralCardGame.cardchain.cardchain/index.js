    ...
    actions: {
        ...
        async QueryQCardContent({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryQCardContent', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryQUser({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryQUser', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryQCardchainInfo({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryQCardchainInfo', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryQVotingResults({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryQVotingResults', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryQVotableCards({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryQVotableCards', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryQCards({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryQCards', payload: { options: { all }, params: { ...key }, query } });
        ...
        async sendMsgReportCopyrightViolation({ rootGetters }, { value, fee = [], memo = '' }) {
            try {
                const txClient = await initTxClient(rootGetters);
                const msg = await txClient.msgReportCopyrightViolation(value);
                const result = await txClient.signAndBroadcast([msg], { fee: { amount: fee,
                        gas: "200000" }, memo });
                return result;
            }
                ...
                if (e == MissingWalletError) {
                    throw new SpVuexError('TxClient:MsgReportCopyrightViolation:Init', 'Could not initialize signing client. Wallet is required.');
                }
                else {
                    throw new SpVuexError('TxClient:MsgReportCopyrightViolation:Send', 'Could not broadcast Tx: ' + e.message);
                }
            }
        },
        ...
        async MsgReportCopyrightViolation({ rootGetters }, { value }) {
            try {
                const txClient = await initTxClient(rootGetters);
                const msg = await txClient.msgReportCopyrightViolation(value);
                return msg;
                ...
                if (e == MissingWalletError) {
                    ...
                    throw new SpVuexError('TxClient:MsgReportCopyrightViolation:Create', 'Could not create message: ' + e.message);
                }
            }
        },
